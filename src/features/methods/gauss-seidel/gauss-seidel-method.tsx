...
import {
  gaussSeidel,
  type GaussSeidelParams,
  type GaussSeidelResult,
} from '@/utils/algorithms/gauss-seidel'
...
import GaussSeidelTable from '@/features/methods/gauss-seidel/gauss-seidel-table'
...
export default function GaussSeidelMethod() {
  ...
  const handleSolve = async () => {
    ...
    try {
      const params: GaussSeidelParams = {
        matrix,
        vector,
        initialGuess,
      ...
      const gaussSeidelResult = gaussSeidel(params)
      setResult(gaussSeidelResult)
    } catch (err) {
  ...
  const exportResults = () => {
    ...
    a.href = url
    a.download = 'gauss_seidel_resultados.csv'
    a.click()
    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-muted-foreground mt-2'>
          Método iterativo que utiliza los valores más recientes para resolver
          sistemas Ax = b
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configura los parámetros de convergencia para el método de
            Gauss-Seidel
          </CardDescription>
        </CardHeader>
      ...
      {result && (
        <div className='space-y-6'>
          ...
          <Card>
            <CardHeader>
              <div className='flex items-center justify-between'>
                <CardTitle>Resultados del Método de Gauss-Seidel</CardTitle>
                ...
                <div>
                  ...
                  <div className='space-y-2 rounded-lg bg-gray-50 p-4'>
                    ...
                    <p className='text-sm text-blue-700'>
                      💡 Gauss-Seidel generalmente converge más rápido que
                      Jacobi para matrices con dominancia diagonal
                    </p>
                    {!result.canConverge && (
          ...
          {/* Iterations Table */}
          <GaussSeidelTable iterations={result.iterations} />
        </div>
      )}
    </div>
  )
