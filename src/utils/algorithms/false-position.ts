...
export function falsePositionMethod(
  ...
  const iterationData: FalsePositionResult['iterationData'] = []
  ...
  try {
    ...
    // Calculate initial xr using false position formula
    let xr = xi - (fxi * (xs - xi)) / (fxs - fxi)
    let fxr = evaluateFunction(functionExpression, xr)
    let error = tolerance + 1 // Initialize error greater than tolerance

    while (
      error > tolerance &&
      Math.abs(fxr) > tolerance &&
      ...
      // Calculate new xr using false position formula
      xr = xi - (fxi * (xs - xi)) / (fxs - fxi)
      fxr = evaluateFunction(functionExpression, xr)
    ...
    if (Math.abs(fxr) < tolerance) {
      return {
        ...
        converged: true,
        message: `${xr.toFixed(6)} es raíz de f(x)`,
        iterationData,
      }
      ...
      return {
        ...
        iterations: iteration,
        converged: true,
        message: `${xr.toFixed(6)} es una aproximación de una raíz de f(x) con una tolerancia = ${tolerance}`,
        iterationData,
      }
    } else {
      return {
